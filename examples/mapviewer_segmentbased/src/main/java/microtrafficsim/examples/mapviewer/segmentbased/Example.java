...
public class Example {
    ...
    private static final float STREET_SCALE_NORMAL = (float) (1.0 / Math.pow(2, 19));


    static SegmentBasedVisualization createVisualization(SegmentLayerProvider provider) {
    ...
    static Set<LayerDefinition> getLayerDefinitions() {
        ...
        if (geomShader != null) progStreets.addSource(GL3.GL_GEOMETRY_SHADER, geomShader);
        ...
        // create layers
        int index = 0;
        layers.add(genLayer("streets:other:outline",    index++, "streets:other",    otherOutline));
        layers.add(genLayer("streets:primary:outline",  index++, "streets:primary",  primaryOutline));
        layers.add(genLayer("streets:trunk:outline",    index++, "streets:trunk",    trunkOutline));
        layers.add(genLayer("streets:motorway:outline", index++, "streets:motorway", motorwayOutline));
        layers.add(genLayer("streets:other:base",       index++, "streets:other",    otherInner));
        layers.add(genLayer("streets:primary:base",     index++, "streets:primary",  primaryInner));
        layers.add(genLayer("streets:trunk:base",       index++, "streets:trunk",    trunkInner));
        layers.add(genLayer("streets:motorway:base",    index++, "streets:motorway", motorwayInner));

        return layers;
    }

    private static LayerDefinition genLayer(String name, int index, String feature, Style style) {
        return new LayerDefinition(name, index, new FeatureSegmentLayerSource(feature, style));
    }

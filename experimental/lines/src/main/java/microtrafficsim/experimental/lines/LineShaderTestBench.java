...
class LineShaderTestBench implements Renderer {
    private static final Logger logger = LoggerFactory.getLogger(LineShaderTestBench.class);
    ...
    private void setKeyCommands(OrthoInputController controller) {
        /* line angle */
        controller.addKeyCommand(KeyEvent.EVENT_KEY_PRESSED, KeyEvent.VK_UP, e -> {
            int a  = (e.getModifiers() & KeyEvent.CTRL_MASK) != 0 ? 1 : 5;
            angle1 = (angle1 + a) % 360;
            if (model != null) model.setAngle1(angle1);
        });

        controller.addKeyCommand(KeyEvent.EVENT_KEY_PRESSED, KeyEvent.VK_DOWN, e -> {
            int a  = (e.getModifiers() & KeyEvent.CTRL_MASK) != 0 ? 1 : 5;
            angle1 = (angle1 - a) % 360;
            if (model != null) model.setAngle1(angle1);
        });

        controller.addKeyCommand(KeyEvent.EVENT_KEY_PRESSED, KeyEvent.VK_LEFT, e -> {
            int a  = (e.getModifiers() & KeyEvent.CTRL_MASK) != 0 ? 1 : 5;
            angle2 = (angle2 + a) % 360;
            if (model != null) model.setAngle2(angle2);
        });

        controller.addKeyCommand(KeyEvent.EVENT_KEY_PRESSED, KeyEvent.VK_RIGHT, e -> {
            int a  = (e.getModifiers() & KeyEvent.CTRL_MASK) != 0 ? 1 : 5;
            angle2 = (angle2 - a) % 360;
            if (model != null) model.setAngle2(angle2);
        });

        controller.addKeyCommand(KeyEvent.EVENT_KEY_RELEASED, KeyEvent.VK_ESCAPE, e -> {
        ...
        /* line blur */
        controller.addKeyCommand(KeyEvent.EVENT_KEY_PRESSED, KeyEvent.VK_PAGE_UP, e -> {
            lineblur = lineblur + 1;
            context.addTask(c -> {
                uLineBlur.set(lineblur);
                return null;
            });
        });

        controller.addKeyCommand(KeyEvent.EVENT_KEY_PRESSED, KeyEvent.VK_PAGE_DOWN, e -> {
        ...
        /* screenshot */
        controller.addKeyCommand(KeyEvent.EVENT_KEY_RELEASED, KeyEvent.VK_F12, e -> asyncScreenshot(context));

        /* exit */
        controller.addKeyCommand(KeyEvent.EVENT_KEY_RELEASED, KeyEvent.VK_F5, e -> context.addTask(c -> {
            reloadShaders(c.getDrawable().getGL().getGL3());
            return null;
        }));
    }
    ...
    private void reloadShaders(GL3 gl) {
        ...
        if (!error) System.out.println("reloading shaders: success");
    }
    ...
    private void initGeometry(GL3 gl) {
        ...
        instances = new ModelInstance[]{
                new ModelInstance(model, new Vec3f(-420, -100.f, 0), 100.f, FG_COLOR, GL3.GL_FILL, 40.f,
                                  CAP_TYPE_BUTT, JOIN_TYPE_NONE, true),
                new ModelInstance(model, new Vec3f(-420, 100.f, 0), 100.f, FG_COLOR, GL3.GL_LINE, 40.f,
                                  CAP_TYPE_BUTT, JOIN_TYPE_NONE, false),
                new ModelInstance(model, new Vec3f(-140, -100.f, 0), 100.f, FG_COLOR, GL3.GL_FILL, 40.f,
                                  CAP_TYPE_SQUARE, JOIN_TYPE_MITER, true),
                new ModelInstance(model, new Vec3f(-140, 100.f, 0), 100.f, FG_COLOR, GL3.GL_LINE, 40.f,
                                  CAP_TYPE_SQUARE, JOIN_TYPE_MITER, false),
                new ModelInstance(model, new Vec3f(140, -100.f, 0), 100.f, FG_COLOR, GL3.GL_FILL, 40.f,
                                  CAP_TYPE_ROUND, JOIN_TYPE_BEVEL, true),
                new ModelInstance(model, new Vec3f(140, 100.f, 0), 100.f, FG_COLOR, GL3.GL_LINE, 40.f,
                                  CAP_TYPE_ROUND, JOIN_TYPE_BEVEL, false),
                new ModelInstance(model, new Vec3f(420, -100.f, 0), 100.f, FG_COLOR, GL3.GL_FILL, 40.f,
                                  CAP_TYPE_ROUND, JOIN_TYPE_ROUND, true),
                new ModelInstance(model, new Vec3f(420, 100.f, 0), 100.f, FG_COLOR, GL3.GL_LINE, 40.f,
                                  CAP_TYPE_ROUND, JOIN_TYPE_ROUND, false)};
    }

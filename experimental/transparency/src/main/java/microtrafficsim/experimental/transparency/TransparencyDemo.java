...
class TransparencyDemo implements Renderer {
    ...
    private static final Color CLEAR_COLOR = new Color(0.75f, 0.75f, 0.75f, 1.0f);

    private static final Resource DEFAULT_VERT_SHADER   = new PackagedResource(TransparencyDemo.class, "default.vs");
    private static final Resource DEFAULT_FRAG_SHADER   = new PackagedResource(TransparencyDemo.class, "default.fs");
    private static final Resource OIT_ACCUM_VERT_SHADER = new PackagedResource(TransparencyDemo.class, "oit_accum.vs");
    private static final Resource OIT_ACCUM_FRAG_SHADER = new PackagedResource(TransparencyDemo.class, "oit_accum.fs");
    private static final Resource OIT_BLEND_VERT_SHADER = new PackagedResource(TransparencyDemo.class, "oit_blend.vs");
    private static final Resource OIT_BLEND_FRAG_SHADER = new PackagedResource(TransparencyDemo.class, "oit_blend.fs");

    private static final int PRIMITIVE_RESTART_INDEX = 0xFFFFFFFF;

    private static final float[] GEOM_VERTICES = {
            -1.0f,  1.0f, -0.5f,
            -1.0f, -1.0f, -0.5f,
             1.0f,  1.0f, -0.5f,
             1.0f, -1.0f, -0.5f,
    ...
    private static final float[] GEOM_COLORS = {
            1.0f, 0.0f, 0.0f, 0.5f,
            1.0f, 0.0f, 0.0f, 0.5f,
            1.0f, 0.0f, 0.0f, 0.5f,
            1.0f, 0.0f, 0.0f, 0.5f,
    ...
    private static final int[] GEOM_INDICES_FWD = {
            0, 1,  2,  3, PRIMITIVE_RESTART_INDEX,
            4, 5,  6,  7, PRIMITIVE_RESTART_INDEX,
            8, 9, 10, 11
    };

    private static final int[] GEOM_INDICES_BWD = {
            8, 9, 10, 11, PRIMITIVE_RESTART_INDEX,
            4, 5,  6,  7, PRIMITIVE_RESTART_INDEX,
            0, 1,  2,  3
    };

    private static final int TEXUNIT_FB_ACCUMULATION = 0;
    private static final int TEXUNIT_FB_REVEALAGE    = 1;

    // framebuffers and textures
    private int framebuffer;
    private int fbTexAccum;
    private int fbTexReveal;

    // matrices
    private Mat4f         modelFwd;
    private Mat4f         modelBwd;
    ...
    private TransparencyDemo(GLWindow window, RenderContext context) {
        this.window      = window;
    ...
    private void initShaders(GL3 gl) {
        ...
        uMatrixViewProjection
                = (UniformMat4f) context.getUniformManager().putGlobalUniform("u_viewprojection", DataTypes.FLOAT_MAT4);
        ...
        // accumulation program
        {
            ...
            UniformVec2f uZPlane = (UniformVec2f) accumShader.getUniform("u_zplane");
    ...
    private void initFrameBuffers(GL3 gl) {
        ...
        gl.glTexImage2D(GL3.GL_TEXTURE_2D, 0, GL3.GL_RGBA16F, width, height, 0, GL3.GL_RGBA, GL3.GL_UNSIGNED_SHORT,
                        null);
    ...
    private void setMatrices() {
        ...
        Mat4f viewprojection = Mat4f.mul(projection, view);
        modelFwd             = Mat4f.identity().rotate(0, 1, 0, 10).translate(10, 0, 0).scale(5, 5, 5);

        modelBwd = Mat4f.identity().rotate(0, 1, 0, -10).translate(-10, 0, 0).scale(5, 5, 5);
    ...
    private class KeyListenerImpl implements KeyListener {
        ...
        public void keyReleased(KeyEvent e) {
            ...
            switch (e.getKeyCode()) {
            case KeyEvent.VK_F1: mode = Mode.BLEND_NONE;             break;
            case KeyEvent.VK_F2: mode = Mode.BLEND_ALPHA;            break;
            case KeyEvent.VK_F3: mode = Mode.BLEND_WBOIT;            break;
            case KeyEvent.VK_F5: mode = Mode.BUFFER_WBOIT_ACCUM;     break;
            case KeyEvent.VK_F6: mode = Mode.BUFFER_WBOIT_REVEALAGE; break;
            ...
            if (prev != mode) System.out.println("Current Mode: " + mode.name);
        }
    }
}

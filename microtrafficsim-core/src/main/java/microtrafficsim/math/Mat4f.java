...
public class Mat4f {
    ...
    public Mat4f(float a11, float a12, float a13, float a14,
        ...
        raw[ 8] = a31;  raw[ 9] = a32;  raw[10] = a33;  raw[11] = a34;
        raw[12] = a41;  raw[13] = a42;  raw[14] = a43;  raw[15] = a44;
    }
    ...
    public Mat4f makeLookInDirection(Vec3f eye, Vec3f dir, Vec3f up) {
        ...
        this.raw[ 8] = -fwdX;  this.raw[ 9] = -fwdY;  this.raw[10] = -fwdZ; this.raw[11] = 0;
        this.raw[12] =     0;  this.raw[13] =     0;  this.raw[14] = 0; this.raw[15] = 1;
    ...
    public Mat4f add(Mat4f other) {
        ...
        raw[12] += other.raw[12];  raw[13] += other.raw[13];  raw[14] += other.raw[14];  raw[15] += other.raw[15];
        return this;
    }
    ...
    public Vec4f mul(Vec4f v) {
        return new Vec4f(this.raw[ 0] * v.x + this.raw[ 1] * v.y + this.raw[ 2] * v.z + this.raw[ 3] * v.z,
    ...
    public Mat4f mul(float scalar) {
        ...
        raw[12] *= scalar;  raw[13] *= scalar;  raw[14] *= scalar;  raw[15] *= scalar;
        return this;
    }
    ...
    public Mat4f scale(float sx, float sy, float sz) {
        ...
        this.raw[12] *= sx;  this.raw[13] *= sy;  this.raw[14] *= sz;
        return this;
    }
    ...
    /**
     * Inverts this matrix. See {@link Mat4f#invert()} for a possibly more performant version, depending on the contents
     * of the matrix.
     *
     * @return this matrix.
     */
    ...
    public static Mat4f add(Mat4f a, Mat4f b) {
        return new Mat4f(a.raw[ 0] + b.raw[ 0], a.raw[ 1] + b.raw[ 1], a.raw[ 2] + b.raw[ 2], a.raw[ 3] + b.raw[ 3],
                a.raw[ 4] + b.raw[ 4], a.raw[ 5] + b.raw[ 5], a.raw[ 6] + b.raw[ 6], a.raw[ 7] + b.raw[ 7],
                a.raw[ 8] + b.raw[ 8], a.raw[ 9] + b.raw[ 9], a.raw[10] + b.raw[10], a.raw[11] + b.raw[11],
                a.raw[12] + b.raw[12], a.raw[13] + b.raw[13], a.raw[14] + b.raw[14], a.raw[15] + b.raw[15]);
    }
    ...
    public static Mat4f mul(Mat4f a, Mat4f b) {
        return new Mat4f(a.raw[ 0] * b.raw[0] + a.raw[ 1] * b.raw[4] + a.raw[ 2] * b.raw[ 8] + a.raw[ 3] * b.raw[12],
    ...
    /**
     * Multiplies the specified matrix with the specified scalar value and retunrs the result in a new matrix.
     *
     * @param a      the matrix to multiply with.
     * @param scalar the scalar value to multiply with.
     * @return the result of this multiplication, i.e. {@code a * scalar}.
     */
    public static Mat4f mul(Mat4f a, float scalar) {
        return new Mat4f(a.raw[ 0] * scalar, a.raw[ 1] * scalar, a.raw[ 2] * scalar, a.raw[ 3] * scalar,
                a.raw[ 4] * scalar, a.raw[ 5] * scalar, a.raw[ 6] * scalar, a.raw[ 7] * scalar,
                a.raw[ 8] * scalar, a.raw[ 9] * scalar, a.raw[10] * scalar, a.raw[11] * scalar,
                a.raw[12] * scalar, a.raw[13] * scalar, a.raw[14] * scalar, a.raw[15] * scalar);
    }
    ...
    public static Mat4f scale(Mat4f m, Vec3f v) {
        return new Mat4f(m.raw[ 0] * v.x, m.raw[ 1] * v.y, m.raw[ 2] * v.z, m.raw[ 3],
                m.raw[ 4] * v.x, m.raw[ 5] * v.y, m.raw[ 6] * v.z, m.raw[ 7],
                m.raw[ 8] * v.x, m.raw[ 9] * v.y, m.raw[10] * v.z, m.raw[11],
                m.raw[12] * v.x, m.raw[13] * v.y, m.raw[14] * v.z, m.raw[15]);
    }
    ...
    public static Mat4f scale(Mat4f m, float x, float y, float z) {
        return new Mat4f(m.raw[ 0] * x, m.raw[ 1] * y, m.raw[ 2] * z, m.raw[ 3],
                m.raw[ 4] * x, m.raw[ 5] * y, m.raw[ 6] * z, m.raw[ 7],
                m.raw[ 8] * x, m.raw[ 9] * y, m.raw[10] * z, m.raw[11],
                m.raw[12] * x, m.raw[13] * y, m.raw[14] * z, m.raw[15]);
    }
    ...
    public static Mat4f rotateX(Mat4f m, float angle) {
        ...
        return new Mat4f(m.raw[ 0], m.raw[ 1] * c + m.raw[ 2] * s, -m.raw[ 1] * s + m.raw[ 2] * c, m.raw[ 3],
                m.raw[ 4], m.raw[ 5] * c + m.raw[ 6] * s, -m.raw[ 5] * s + m.raw[ 6] * c, m.raw[ 7],
                m.raw[ 8], m.raw[ 9] * c + m.raw[10] * s, -m.raw[ 9] * s + m.raw[10] * c, m.raw[11],
                m.raw[12], m.raw[13] * c + m.raw[14] * s, -m.raw[13] * s + m.raw[14] * c, m.raw[15]);
    }
    ...
    public static Mat4f rotateY(Mat4f m, float angle) {
        ...
        return new Mat4f(m.raw[ 0] * c - m.raw[ 2] * s, m.raw[ 1], m.raw[ 0] * s + m.raw[ 2] * c, m.raw[ 3],
                m.raw[ 4] * c - m.raw[ 6] * s, m.raw[ 5], m.raw[ 4] * s + m.raw[ 6] * c, m.raw[ 7],
                m.raw[ 8] * c - m.raw[10] * s, m.raw[ 9], m.raw[ 8] * s + m.raw[10] * c, m.raw[11],
                m.raw[12] * c - m.raw[14] * s, m.raw[13], m.raw[12] * s + m.raw[14] * c, m.raw[15]);
    }
    ...
    public static Mat4f rotateZ(Mat4f m, float angle) {
        ...
        return new Mat4f(m.raw[ 0] * c + m.raw[ 1] * s, -m.raw[ 0] * s + m.raw[ 1] * c, m.raw[ 2], m.raw[ 3],
                m.raw[ 4] * c + m.raw[ 5] * s, -m.raw[ 4] * s + m.raw[ 5] * c, m.raw[ 6], m.raw[ 7],
                m.raw[ 8] * c + m.raw[ 9] * s, -m.raw[ 8] * s + m.raw[ 9] * c, m.raw[10], m.raw[11],
                m.raw[12] * c + m.raw[13] * s, -m.raw[12] * s + m.raw[13] * c, m.raw[14], m.raw[15]);
    }
    ...
    public static Mat4f transpose(Mat4f m) {
        return new Mat4f(m.raw[0], m.raw[4], m.raw[ 8], m.raw[12],
    ...
    /**
     * Transposes the upper left 3x3 matrix of the specified matrix, leaving all other elements as they are.
     *
     * @param m the matrix to be transposed.
     * @return the (partially) transposed matrix.
     */
    public static Mat4f transpose3(Mat4f m) {
        return new Mat4f(m.raw[ 0], m.raw[ 4], m.raw[ 8], m.raw[ 3],
    ...
    public static Mat4f invertAffine(Mat4f m) {
        ...
        return new Mat4f(m.raw[0], m.raw[4], m.raw[ 8], -dot1t,

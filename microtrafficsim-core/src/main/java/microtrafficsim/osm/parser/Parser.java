...
public class Parser {
	...
	private DataSet extract(File file) throws XMLStreamException, IOException {
		...
		MultiPassParserBaseEventHandler handler = new MultiPassParserBaseEventHandler(
				datastore, features, nodeManager, wayManager, relations);
		...
		if (!handler.hasRequiredPrimitives()) {
			logger.warn("not all required elements could be parsed");
		}

 ...
 */
public class GraphWayComponent extends Component
		implements Mergeable<GraphWayComponent>, Splittable, Reversible {
	...
	public boolean forwardMergeable(DataSet dataset, WayLayout layout, NodeEntity node, GraphWayComponent other) {
		...
		if (needsAtoB) {
			...
			if ((!this.from.contains(ab)) && (!other.to.contains(ab)))
				return false;
		}
		
		if (needsBtoA) {
			Connector ba = new Connector(node, (WayEntity) other.entity, (WayEntity) this.entity);
			if ((!this.to.contains(ba)) && (!other.from.contains(ba)))
				return false;
		}
	...
	public void merge(DataSet dataset, MergePoint mergepoint, GraphWayComponent other) {
		...
		if (keep.nodes[0] == remove.nodes[remove.nodes.length - 1]) {
			...
			if (this.from.contains(kr))
				this.cyclicStartToEnd = true;
				
			if (this.to.contains(rk))
				this.cyclicEndToStart = true;
		...
		for (Connector c : other.uturn) {
			if (c.via == mergepoint.node)
				continue;	// ignore u-turns on merge-point
	...
	public Component clone(Entity e) {
		return new GraphWayComponent((WayEntity) e,
	...
	public void split(DataSet dataset, WayEntity[] splits, int[] splitpoints) {
		...
		if (way.nodes[0] == way.nodes[way.nodes.length - 1]) {
			...
			if (this.cyclicStartToEnd)
				Connectors.add(Connectors.create(via, splits[0], splits[splits.length - 1]));
			
			if (this.cyclicEndToStart)
				Connectors.add(Connectors.create(via, splits[splits.length - 1], splits[0]));
		}
		...
		for (long x : xpts) {
			...
			for (WayEntity segment : splits)
				if (x == segment.nodes[0] || x == segment.nodes[segment.nodes.length - 1])
					adjacent.add(segment);
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof GraphWayComponent))
			return false;

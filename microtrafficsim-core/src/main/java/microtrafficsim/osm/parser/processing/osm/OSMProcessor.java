...
import microtrafficsim.osm.parser.processing.osm.sanitizer.OSMDataSetSanitizer;
import microtrafficsim.osm.parser.processing.osm.sanitizer.OSMStreetGraphSanitizer;
...
public class OSMProcessor implements Processor {
	...
	public static void setupGraphNodeComponents(DataSet dataset, int idxStreetGraph) {
		for (WayEntity way : dataset.ways.values()) {
			...
			for (long ref : way.nodes) {
				...
				if (node == null)
					throw new RuntimeException("node with id " + ref + " needed but not found");
	...
	private void unifySplit(DataSet dataset) {
		...
		for (WayEntity way : dataset.ways.values()) {
			...
			for (int i = 1; i < way.nodes.length - 1; i++) {
				...
				if (dataset.nodes.get(ref).get(GraphNodeComponent.class).ways.count() > 1) {
					splitpoints.add(i);
				}
			}
	...
	public void execute(Parser parser, DataSet dataset) {
		...
		logger.debug("dataset before unification: " + dataset.nodes.size() + " nodes, " + dataset.ways.size() + " ways");
		...
		logger.debug("dataset after unification:  " + dataset.nodes.size() + " nodes, " + dataset.ways.size() + " ways");

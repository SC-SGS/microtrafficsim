...
public class Ways {
	...
	public static boolean isConnectionPossible(WayLayout layout, OnewayInfo a, OnewayInfo b) {
		switch (layout) {
		case START_START:		// <---o--->
			return !(a == OnewayInfo.FORWARD || b == OnewayInfo.BACKWARD);

		case START_END:			// <---o<---
			return !(a == OnewayInfo.FORWARD || b == OnewayInfo.FORWARD);

		case END_START:			// --->o--->
			return !(a == OnewayInfo.BACKWARD || b == OnewayInfo.BACKWARD);

		case END_END:			// --->o<---
			return !(a == OnewayInfo.BACKWARD || b == OnewayInfo.FORWARD);

		case T_FROM_A:
			return b != OnewayInfo.BACKWARD;

		case T_TO_A:
			return b != OnewayInfo.FORWARD;

		case T_FROM_B:
			return a != OnewayInfo.FORWARD;

		case T_TO_B:
			return a != OnewayInfo.BACKWARD;
	...
	public static MergePoint mergepoint(DataSet dataset, NodeEntity node, WayEntity a, WayEntity b) {
		...
		if (layout.isAligned()) {
			for (Class<? extends Component> type : types)
				if (!forwardMergeable(dataset, layout, node, a.get(type), b.get(type)))
					return null;
		} else {
			for (Class<? extends Component> type : types)
				if (!reverseMergeable(dataset, layout, node, a.get(type), b.get(type)))
					return null;
		}
	...
	private static boolean forwardMergeable(DataSet dataset, WayLayout layout, NodeEntity node, Component a, Component b) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static boolean reverseMergeable(DataSet dataset, WayLayout layout, NodeEntity node, Component a, Component b) {
		if (a instanceof Mergeable && b instanceof Mergeable)

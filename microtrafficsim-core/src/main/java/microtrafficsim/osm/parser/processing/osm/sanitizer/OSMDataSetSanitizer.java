...
public class OSMDataSetSanitizer implements Processor {
	...
	private void sanitizeWay(WayEntity way) {
		...
		if (way.nodes.length > 0) {
			...
			if (len != way.nodes.length)
				way.nodes = Arrays.copyOf(nodes, len);
		}
		
		// sanitize components
		SanitizerWayComponent sancomp = way.get(SanitizerWayComponent.class);
		if (sancomp != null) {
			sanitizeStreetComponent(way.get(StreetComponent.class), sancomp);
		}
	}
	...
	private void sanitizeStreetComponent(StreetComponent streetcomp, SanitizerWayComponent sancomp) {
		...
		if (streetcomp.streettype == null)
			streetcomp.streettype = StreetType.ROAD;
		...
		// XXX TEMPORARY
		if (streetcomp.lanes.forward > 0)
			streetcomp.lanes.forward = 1;
		
		if (streetcomp.lanes.backward > 0)
			streetcomp.lanes.backward = 1;

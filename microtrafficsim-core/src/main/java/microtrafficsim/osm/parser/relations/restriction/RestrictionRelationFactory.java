...
public class RestrictionRelationFactory implements RelationFactory {
    ...
    private Restriction getRestrictionType(Relation r) {
        ...
        case "no_right_turn":    return Restriction.NO_RIGHT_TURN;
        case "no_left_turn":     return Restriction.NO_LEFT_TURN;
        case "no_u_turn":        return Restriction.NO_U_TURN;
        case "no_straight_on":   return Restriction.NO_STRAIGHT_ON;
        case "only_right_turn":  return Restriction.ONLY_RIGHT_TURN;
        case "only_left_turn":   return Restriction.ONLY_LEFT_TURN;
        case "only_straight_on": return Restriction.ONLY_STRAIGHT_ON;
        case "no_entry":         return Restriction.NO_ENTRY;
        case "no_exit":          return Restriction.NO_EXIT;
        default:                 return null;
        }
    }
    ...
    public RestrictionRelation create(Relation r) {
        ...
        for (RelationMember m : r.members) {
            switch (m.role) {
            case "from": from.add(m.ref); break;
            case "to":   to.add(m.ref); break;
            case "via":  via.add(m.ref); break;
            default:     // ignore
            }
        }

        Primitive.Type viaType      = Primitive.Type.NODE;
        if (via.size() > 1) viaType = Primitive.Type.WAY;

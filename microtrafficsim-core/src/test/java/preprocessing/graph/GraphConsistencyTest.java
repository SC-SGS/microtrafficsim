...
public class GraphConsistencyTest {
	 ...
	 */
	private static final Predicate<Way> SG_WAY_MATCHER = w -> w.visible
				&& w.tags.get("highway") != null
				&& !w.tags.get("highway").equals("elevator")
	...
	private static ArrayList<WaySliceMapping> getMatches(WayEntity from, WayEntity to) {
		...
		// compare features
		if (!from.features.equals(to.features))
			return matches;
		...
		if ((scf == null) != (sct == null))
			return matches;
		if (scf != null && !from.get(StreetComponent.class).equals(to.get(StreetComponent.class)))
			return matches;
	 ...
	 */
	private static boolean streetConnectionRestricted(HashSet<RestrictionRelation> restrictions,
			long via, long from, long to) {
	...
	private static void getAllMappingsRecursive(int offset, int last, HashSet<WaySliceMapping> slices,
			LinkedList<WaySliceMapping> stack, ArrayList<ArrayList<WaySliceMapping>> result) {
	...
	private static ArrayList<Long> getIntersectionNodes(WaySliceMapping a, WaySliceMapping b) {
		...
		for (int i = a.from.start; i <= a.from.end; i++)
			for (int j = b.from.start; j <= b.from.end; j++)
				if (a.from.way.nodes[i] == b.from.way.nodes[j])
					intersections.add(a.from.way.nodes[i]);
	...
	private static boolean containsConnectorByIDs(HashSet<Connector> set, long via, long from, long to) {
		for (Connector e : set)
			if (e.via.id == via && e.from.id == from && e.to.id == to)
				return true;
	...
	private static void assertDataSetConsistent(DataSet dataset) {
		...
		for (WayEntity way : dataset.ways.values()) {
			...
			if (streetcomp != null) {
				...
				} else if (streetcomp.oneway == OnewayInfo.FORWARD ||
						streetcomp.oneway == OnewayInfo.REVERSIBLE) {
					assertTrue(streetcomp.lanes.forward > 0);
					assertTrue(streetcomp.lanes.backward == 0);
					assertTrue(streetcomp.maxspeed.forward > 0);
					assertTrue(streetcomp.maxspeed.backward == 0);
	...
	private static void assertConnectorPropagation(DataSet from, DataSet to, ArrayList<WaySliceMapping> mapping) {
		...
		for (WaySliceMapping prev = null, curr = iter.next(), next = null; curr != null;
				prev = curr, curr = next, next = iter.hasNext() ? iter.next() : null) {
			
			// make sure connectors prev<-->curr exist on to
			if (prev != null)
				assertConnectorsExistBetweenSlices(prev, curr);
			
			// make sure connectors curr<-->next exist on to
			if (next != null)
				assertConnectorsExistBetweenSlices(curr, next);
	 ...
	 * 						of the slices.
	 */
	private static void assertConnectorsEqualOnIntersection(WaySliceMapping a, WaySliceMapping b,
			long node, DataSet datasetFrom, DataSet datasetTo) {
		...
		if (a.to.way != b.to.way) {
			...
			// basic consistency
			assertEquals(containsConnectorByIDs(gwcFromA.from, fid, afid, bfid), containsConnectorByIDs(gwcFromB.to, fid, afid, bfid));
			assertEquals(containsConnectorByIDs(gwcFromA.to, fid, bfid, afid), containsConnectorByIDs(gwcFromB.from, fid, bfid, afid));
			assertEquals(containsConnectorByIDs(gwcToA.from, tid, atid, btid), containsConnectorByIDs(gwcToB.to, tid, atid, btid));
			assertEquals(containsConnectorByIDs(gwcToA.to, tid, btid, atid), containsConnectorByIDs(gwcToB.from, tid, btid, atid));
			...
			if (possibleFromAB && possibleToAB)
				assertEquals(containsConnectorByIDs(gwcFromA.from, fid, afid, bfid), containsConnectorByIDs(gwcToA.from, tid, atid, btid));
			if (!possibleToAB)
				assertFalse(containsConnectorByIDs(gwcToA.from, tid, atid, btid));
			if (!possibleFromAB)
				assertFalse(containsConnectorByIDs(gwcFromA.from, fid, afid, bfid));
			
			if (possibleFromBA && possibleToBA)
				assertEquals(containsConnectorByIDs(gwcFromA.to, fid, bfid, afid), containsConnectorByIDs(gwcToA.to, tid, btid, atid));
			if (!possibleToBA)
				assertFalse(containsConnectorByIDs(gwcToA.to, tid, btid, atid));
			if (!possibleFromBA)
				assertFalse(containsConnectorByIDs(gwcFromA.to, fid, bfid, afid));
		...
		// else if to.way is the same and cyclic
			...
			// intersection inside to.way, all required connectors automatically present
		}
	}
			...
			if (gnc != null && tnc != null) {
				...
				for (Iterator<MultiSet.Entry<WayEntity>> gIter = gnc.ways.entryIterator(); gIter.hasNext();) {
					...
					for (Iterator<MultiSet.Entry<WayEntity>> tIter = gnc.ways.entryIterator(); tIter.hasNext();) {
						...
						if (gEntry.getCount() == tEntry.getCount() && gEntry.getElement().id == tEntry.getElement().id) {
							found = true;
							break;
						}
					}
			...
			for (WayEntity a : tnc.ways) {
				...
				for (WayEntity b : tnc.ways) {
					...
					// make sure either there is either a Connector or a Restriction (XOR)
						...
						Predicate<Connector> cmatchAB = c -> c.via.id == node.id
								&& c.from.id == a.id
								&& c.to.id == b.id;

						Predicate<Connector> cmatchBA = c -> c.via.id == node.id
								&& c.from.id == b.id
								&& c.to.id == a.id;
						...
						if (requiresAB)
							assertTrue("there must be either a connector or a restriction", connectorAtoB != restrictedAB);
						
						if (requiresBA)
							assertTrue("there must be either a connector or a restriction", connectorBtoA != restrictedBA);
					}
				}
			}
		}
	...
	public void stage2GraphMinimalTest() {
		for (NodeEntity node : testdata.stage2.nodes.values()) {
			...
			if (tnc.ways.size() == 2 && tnc.ways.count() == 2) {
				...
				assertNull("way segments separate but should be merged", Ways.mergepoint(testdata.stage2, node, w1, w2));	
			}
		}
	}

...
Options:
    ...
    -i <f> --in=<f>     Specify the path to the csv file folder [default: ../data/csv/total_anger_when_despawning.csv]
    -o <f> --out=<f>    Specify the output file (including path) [default: ../data/pdf/total_anger_when_despawning.pdf]
"""
...
def visualize(input_filepath, output_filepath, show):
    ...
    if totalDespawned > 0:
        ...
        title = 'total anger when despawning\n(%i despawned)' % totalDespawned
        xlabel = 'total anger'
        ylabel = '# vehicles despawned <= total anger (in %)'
